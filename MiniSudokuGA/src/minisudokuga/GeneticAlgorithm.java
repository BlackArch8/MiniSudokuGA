...
public class GeneticAlgorithm {
    ...
    int best;
    float fitness = Integer.MAX_VALUE;
    ...
    void mutation() {
        ...
        if (mutant.genes[mutationPoint].getNumber() == -1) {
            // jika gen tersebut belum terisi, maka isi dengan angka acak antara 1 sampai
            // size
    ...
    void iteration() {
        ...
        while (!termination(iter)) {
            ...
            restChromosomes();
            checkFitness();
            // catat fitness terbaik dan simpan juga kromosom terbaik
            if (fitness > population[best].fitness) {
                fitness = population[best].fitness;
                bestChromosome = new Chromosome(population[best]); 
            }
            iter++;
